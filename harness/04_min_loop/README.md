...
> `mh/` mini-harness 包的第一个器官是主循环：带工具 schema 请求模型 →
> 执行 tool_calls → 结果回填 → 直到无 tool_calls 收敛。34 行循环体，撑起
> 整个包的骨架。规则只有三条：**先执行再收尾、assistant 先落历史再回填、
> 无 tool_calls 即收敛**。

## What

Agent Loop 是"模型意图"与"真实副作用"之间的传送带。一次任务 = 若干个完整
回合：每回合主循环带 tools schema 请求模型；返回 `message.tool_calls` 就
逐个执行，结果以 `role:tool` 回填再进下一回合；返回纯文本即收敛。轮次上限
是最后的安全网。`mh/loop.py` 约 60 行，其中循环体 34 行。
...
也不用 LangChain 的循环：09 预算、10 压缩都要在循环体里插桩，用别人的
循环等于在别人的房子里走线——本系列要的就是"循环里每一行都是自己的"。
...
加 `--trace` 会看到一个彩蛋：qwen3.8 的回复里 `content` 为空、推理在
`"thinking"` 字段——Ollama 把混合推理模型的思考单独存放。`mh/llm.py`
保留该字段原样透传，这正是 10 上下文压缩要治理的对象之一（思考比正文
更占上下文）。
...
两步任务 3 轮收敛，8 是 2.6 倍冗余；16 评测台会统计分布后再定生产值。

**Q2: 为什么 `cwd` 是 run_agent 的参数而不是工具的？**

工作目录是**任务级**约束，模型不该有权选择"在哪执行"——权限模型（07）
的第一课。

**Q3: 收敛判据为什么信协议不信文本？**

与 01 判分铁证同源：确定性信息（tool_calls 有无）优于概率性信息（"任务
完成"字样）。
