...
MOCK=1 python3 fc_vs_react.py    # 离线：预置响应
...
不是。看四个要素：模型是否支持 tools（依赖度）、是否要并行调用、可观测
性要求、token 预算（schema 常驻是实打实的成本）。

**Q2: "FC 更省 token"的结论对吗？**

别背旧结论——本机实测 FC 反而 +75%（schema 每轮重发），好处在结构化与
并行。结论要报自己跑出来的数字。
